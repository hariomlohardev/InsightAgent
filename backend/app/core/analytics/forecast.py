...
def _naive_forecast(ts_df: pd.DataFrame, periods: int, freq: str = "ME") -> Tuple[pd.DataFrame, Dict[str, Any]]:
    # Last value repeated
    ...
    forecast_df = pd.DataFrame({"ds": future_dates, "yhat": [last_val]*periods, "yhat_lower": [last_val*0.9]*periods, "yhat_upper": [last_val*1.1]*periods})
    ...
    return combined, {"method":"naive","warning":"statsforecast not installed — naive last-value fallback","periods":periods}

def _statsforecast_forecast(ts_df: pd.DataFrame, periods: int, freq: str = "ME") -> Tuple[pd.DataFrame, Dict[str, Any]]:
    try:
    ...
    # Choose models
    models = [AutoETS(season_length=12 if freq in ("M","ME","MS") else 1), AutoARIMA(season_length=12 if freq in ("M","ME","MS") else 1)]
    ...
    future = pd.DataFrame({"ds": forecast_df["ds"].values, "y": [None]*periods, "yhat": yhat, "yhat_lower": yhat_lower, "yhat_upper": yhat_upper, "is_forecast": True})
    ...
    if n >= 20:
        ...
        try:
            ...
            col = "AutoETS" if "AutoETS" in pred.columns else [c for c in pred.columns if c not in ("unique_id","ds")][0]
            y_pred = pred[col].values
            y_true = test["y"].values
            mae = float(np.mean(np.abs(y_true - y_pred)))
            rmse = float(np.sqrt(np.mean((y_true - y_pred)**2)))
            metrics = {"mae": round(mae,2), "rmse": round(rmse,2), "holdout": holdout, "method": model_col}
        except Exception as e:
            metrics = {"method": model_col, "backtest_error": str(e)[:100]}
    else:
        metrics = {"method": model_col, "warning": f"Low data ({n} points) — forecast indicative, n<20 so no backtest", "n": n}
    metrics["periods"] = periods
    return combined, metrics

def forecast(df: pd.DataFrame, date_col: Optional[str] = None, metric: Optional[str] = None, periods: int = 3, freq: str = "M", profile: Dict[str, Any] = None) -> Tuple[pd.DataFrame, go.Figure, Dict[str, Any]]:
    """
    ...
    if date_col is None:
        ...
        if not date_col:
            raise ValueError("No date column found — cannot forecast time-series. Available: " + ", ".join(df.columns))
    ...
    if len(ts_df) < 2:
        raise ValueError(f"Not enough time points after resampling ({len(ts_df)}). Need >=2 months of data.")
    warning = None
    if len(ts_df) < 12:
        warning = f"Low data ({len(ts_df)} points <12) — forecast indicative"
    # Try statsforecast, else naive
    try:
        combined, metrics = _statsforecast_forecast(ts_df, periods, freq="ME" if freq in ("M","ME") else freq)
    ...
    fig.add_trace(go.Scatter(x=hist["ds"], y=hist["yhat"], mode="lines+markers", name="history", line=dict(color="#0f172a", width=2)))
    # Forecast line
    fig.add_trace(go.Scatter(x=fc["ds"], y=fc["yhat"], mode="lines+markers", name="forecast", line=dict(color="#2563eb", width=2, dash="dash")))
    # Band
    fig.add_trace(go.Scatter(x=pd.concat([fc["ds"], fc["ds"][::-1]]), y=pd.concat([fc["yhat_upper"], fc["yhat_lower"][::-1]]), fill="toself", fillcolor="rgba(37,99,235,0.15)", line=dict(color="rgba(0,0,0,0)"), name="80% band", hoverinfo="skip"))
    fig.update_layout(title=f"Forecast: {metric} next {periods} ({freq}) — {method} — {warning or ''}", xaxis_title=date_col, yaxis_title=metric, height=380, margin=dict(l=10,r=10,t=40,b=10), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))

...
def segment(df: pd.DataFrame, by: str, metric: str, agg: str = "sum") -> pd.DataFrame:
    ...
    if agg == "count":
        ...
        if agg not in ("sum","mean","median","max","min","count"):
            agg = "sum"
        if agg == "median":
            grouped = df.groupby(by)[metric].median().reset_index().rename(columns={metric: "value"})
        elif agg == "mean":
        ...
        grouped["share"] = (grouped["value"] / total * 100).round(1) if total and agg=="sum" else None
        ...
        if date_col:
            try:
                ...
                for cat, sub in period_agg.groupby(by):
                    ...
                    if len(sub) >= 2:
                        first = sub.iloc[0]["value"] if metric in sub.columns else sub.iloc[0][metric]
                ...
                if growth_map:
                    grouped["growth_pct"] = grouped["category"].map(growth_map) if "category" in grouped.columns else None
...
def plot_segment(df_seg: pd.DataFrame, by_label: str = "category") -> go.Figure:
    ...
    if len(df_seg) <= 8:
        fig = px.bar(df_seg, x="category", y="value", color="share" if "share" in df_seg.columns and df_seg["share"].notna().any() else None, title=f"Segment by {by_label} (share)", text_auto=True)
    else:
        try:
            fig = px.treemap(df_seg, path=["category"], values="value", title=f"Segment by {by_label} — treemap")
        except:

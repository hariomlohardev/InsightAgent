...
PLAN_QUOTAS = {
    ...
    "enterprise": {"datasets": 10000, "queries_per_month": 1000000, "users": 1000, "schedules": 1000},
}
...
def get_billing(ws_id: str = None) -> Dict[str, Any]:
    ...
    if not p.exists():
        # create free
        billing = {"workspace_id": ws_id or get_workspace_id(), "plan": "free", "status": "active", "stripe_customer_id": None, "created_at": datetime.now(timezone.utc).isoformat(), "queries_this_month": 0, "last_reset": datetime.now(timezone.utc).isoformat()}
    ...
    try:
        ...
        # check monthly reset
        try:
            last = datetime.fromisoformat(data.get("last_reset", data.get("created_at", datetime.now(timezone.utc).isoformat())).replace("Z","+00:00"))
...
def get_usage(ws_id: str = None) -> Dict[str, Any]:
    ...
    return {"datasets": datasets, "queries_this_month": queries, "plan": billing.get("plan","free"), "status": billing.get("status","active")}
...
def can_query(ws_id: str = None) -> tuple[bool, str]:
    ...
    if billing.get("queries_this_month",0) >= quotas["queries_per_month"]:
        return False, f"Monthly query limit {quotas['queries_per_month']} reached for plan {plan}, upgrade at /pricing"
    return True, ""

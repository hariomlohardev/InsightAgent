...
try:
    ...
    celery_app = Celery("insight", broker=REDIS_URL, backend=REDIS_URL) if REDIS_URL else Celery("insight", broker="memory://", backend="cache+memory://")
...
if hasattr(celery_app, "task"):
    ...
    try:
        ...
        def run_chat_task(job_id: str, dataset_id: str, query: str, user_id: str = None):
            ...
            _save_job(job_id, {"job_id": job_id, "status":"running", "dataset_id": dataset_id, "query": query})
            try:

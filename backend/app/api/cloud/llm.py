...
async def get_llm(user = Depends(get_current_user)):
    ...
    if not p.exists():
        ...
        return {"provider": os.getenv("LLM_PROVIDER","auto"), "model": os.getenv("LLM_MODEL") or os.getenv("OPENAI_MODEL","gpt-4o-mini"), "ollama_url": os.getenv("OLLAMA_URL","http://localhost:11434")}
    try:
        with open(p) as f:
            data = json.load(f)
        # hide key
...
async def set_llm(body: LLMSetRequest, request: Request, user = Depends(get_current_user)):
    ...
    if provider not in ("openai","groq","gemini","claude","ollama","auto","heuristic"):
        raise HTTPException(status_code=400, detail="provider must be openai|groq|gemini|claude|ollama|auto|heuristic")
    data = {"provider": provider, "model": (body.model or "")[:100], "ollama_url": (body.ollama_url or "http://localhost:11434")[:200], "updated_at": __import__("datetime").datetime.now(__import__("datetime").timezone.utc).isoformat()}
    ...
    if openai_key:
        ...
        if enc_key:
            try:
                ...
                if len(enc_key) < 32:
                    enc_key = base64.urlsafe_b64encode(hashlib.sha256(enc_key.encode()).digest()).decode()

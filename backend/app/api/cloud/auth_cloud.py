...
async def cloud_register(body: CloudRegisterRequest, request: Request):
    ...
    try:
        ...
        pass
    try:
        user = auth_core.create_user(body.email, body.password, role="admin", name=body.name or "", workspace_id=ws_id)
    ...
    return {"workspace_id": ws_id, "user": {"id": user["id"], "email": user["email"], "role": user["role"], "workspace_id": ws_id}, "access_token": token, "token_type": "bearer", "verification": "mock_sent"}
...
async def cloud_login(body: CloudRegisterRequest, request: Request):
    ...
    token = auth_core.create_jwt(user)
    return {"access_token": token, "token_type": "bearer", "user": {"id": user["id"], "email": user["email"], "role": user["role"], "workspace_id": user.get("workspace_id","default")}, "workspace_id": user.get("workspace_id","default")}

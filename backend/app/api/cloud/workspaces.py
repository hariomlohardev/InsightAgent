...
async def get_brand(ws_id: str, user = Depends(get_current_user)):
    ...
    if user.get("workspace_id") != ws_id and user.get("role") != "admin" and user.get("id") != "anon":
        # allow read for brand public
        pass
    p = _brand_path(ws_id)
    if not p.exists():
        return {"workspace_id": ws_id, "app_name": "InsightAgent", "logo_url": "", "primary_color": "#0f172a"}
    try:
        with open(p) as f:
            data = json.load(f)
        return data
...
async def set_brand(ws_id: str, body: BrandRequest, request: Request, user = Depends(get_current_user)):
    ...
    try:
        ...
        plan = get_plan(ws_id)
        if plan not in ("enterprise","team") and os.getenv("CLOUD","false").lower() in ("true","1","yes"):
        ...
        pass
    data = {"workspace_id": ws_id, "app_name": (body.app_name or "InsightAgent")[:50], "logo_url": (body.logo_url or "")[:500], "primary_color": (body.primary_color or "#0f172a")[:20]}
    # validate color hex
    import re

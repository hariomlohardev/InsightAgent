...
async def get_runs(sid: str):
    ...
    if not s:
        raise HTTPException(status_code=404, detail="Schedule not found")
    return {"id": sid, "runs": s.get("runs", []), "last_run": s.get("last_run"), "next_run": s.get("next_run")}
...
async def export_schedule_pdf(sid: str):
    ...
    try:
        ...
        if not dash_id:
            raise HTTPException(status_code=400, detail="Schedule has no dashboard_id — cannot export PDF")

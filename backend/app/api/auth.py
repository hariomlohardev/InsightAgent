...
def get_current_user(credentials: Optional[HTTPAuthorizationCredentials] = Depends(security), x_api_key: Optional[str] = Header(None, alias="X-API-Key"), request: Request = None):
    ...
    if is_cloud:
        try:
            ...
            pass
    return {"id":"anon","email":"anon","role":"editor","name":"Anonymous","workspace_id": anon_ws}
    ...
    def _dep(user: Dict[str, Any] = Depends(get_current_user)):
        if user.get("role") not in roles:
            ...
            if user.get("role") == "admin":
                return user
            raise HTTPException(status_code=403, detail=f"Role {user.get('role')} not allowed, need {roles}")
        return user
    return _dep
        ...
        if existing and requested_role == "admin":
            ...
            if auth_header.lower().startswith("bearer "):
                ...
                if data:
                    ...
                    raise HTTPException(status_code=401, detail="Authentication required for admin creation")
            else:
                raise HTTPException(status_code=403, detail="Only admin can create admin users")
        user = auth_core.create_user(body.email, body.password, role=requested_role, name=body.name or "")
        audit_log("user.register", user, ip=request.client.host if request.client else "")
        # Return safe
        return {"id": user["id"], "email": user["email"], "role": user["role"], "name": user.get("name","")}
...
async def login(body: LoginRequest, request: Request):
    ...
    return {"access_token": token, "token_type":"bearer", "user": {"id": user["id"], "email": user["email"], "role": user["role"], "name": user.get("name","")}}

@router.get("/me")
async def me(user: Dict[str, Any] = Depends(get_current_user)):
    # Return user without hash
    return {"id": user["id"], "email": user.get("email"), "role": user.get("role"), "name": user.get("name","")}

@router.post("/api-key", status_code=201)
async def create_api_key(body: ApiKeyCreate, user: Dict[str, Any] = Depends(get_current_user), request: Request = None):
    ...
    # Return raw once
    return {"id": ak["id"], "name": ak["name"], "api_key": ak["raw"], "scopes": ak["scopes"], "created_at": ak["created_at"]}
...
async def list_api_keys(user: Dict[str, Any] = Depends(get_current_user)):
    ...
    # Don't return hashed
    return [{"id": k["id"], "name": k["name"], "scopes": k.get("scopes"), "created_at": k.get("created_at"), "user_id": k.get("user_id")} for k in keys]

@router.delete("/api-key/{key_id}")
async def delete_api_key(key_id: str, user: Dict[str, Any] = Depends(get_current_user), request: Request = None):
    if user.get("id")=="anon":
        raise HTTPException(status_code=401, detail="Authentication required")
...
async def list_users(user: Dict[str, Any] = Depends(require_role("admin"))):
    ...
    return [{"id": u["id"], "email": u["email"], "role": u["role"], "name": u.get("name",""), "created_at": u.get("created_at")} for u in users]

@router.post("/users/{uid}/role")
async def change_role(uid: str, body: Dict[str, Any], user: Dict[str, Any] = Depends(require_role("admin")), request: Request = None):
    role = body.get("role")
    ...
    audit_log("user.role_change", user, ip=request.client.host if request and request.client else "", extra=f"target={uid} role={role}")

...
async def create_dashboard(body: CreateDashboardRequest, request: Request = None, user = Depends(get_current_user)):
    ...
    try:
        dash = dashboard_service.create_dashboard(body.dataset_id, body.name.strip(), body.description or "")
        audit_log("dashboard.create", user, dashboard_id=dash["id"], dataset_id=body.dataset_id, ip=request.client.host if request and request.client else "")
        return dash
...
@router.get("")
async def list_dashboards(dataset_id: Optional[str] = Query(None), limit: int = Query(100, ge=1, le=200), offset: int = Query(0, ge=0)):
    try:
...
async def delete_dashboard(dash_id: str, request: Request = None, user = Depends(get_current_user)):
    ...
    if not ok:
        raise HTTPException(status_code=404, detail="Dashboard not found")
    audit_log("dashboard.delete", user, dashboard_id=dash_id, ip=request.client.host if request and request.client else "")
    return {"status": "deleted", "id": dash_id}

@router.post("/{dash_id}/widgets")
async def add_widget(dash_id: str, body: AddWidgetRequest, request: Request = None, user = Depends(get_current_user)):
    if user.get("role") not in ("admin","editor"):
    ...
    try:
        ...
        audit_log("dashboard.add_widget", user, dashboard_id=dash_id, ip=request.client.host if request and request.client else "")
        return widget
    except FileNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
...
async def remove_widget(dash_id: str, widget_id: str, request: Request = None, user = Depends(get_current_user)):
    if user.get("role") not in ("admin","editor"):
    ...
    audit_log("dashboard.remove_widget", user, dashboard_id=dash_id, ip=request.client.host if request and request.client else "")
...
async def duplicate_dashboard(dash_id: str):
    ...
    if not dash:
        raise HTTPException(status_code=404, detail="Dashboard not found")
    try:
        new_dash = dashboard_service.create_dashboard(dash["dataset_id"], f"{dash['name']} (copy)", dash.get("description",""))
        # Copy widgets
        for w in dash.get("widgets", []):
            dashboard_service.add_widget(new_dash["id"], {
                "query": w.get("query",""),
                "code": w.get("code",""),
                "result": w.get("result"),
                "chart": w.get("chart"),
...
async def export_dashboard(dash_id: str, format: str = Query("json", pattern="^(json|csv|pdf)$")):
    ...
    if format == "json":
        ...
        return JSONResponse(content=dash, headers={"Content-Disposition": f"attachment; filename=\"{dash_id}.json\""})
    elif format == "csv":
...
async def add_comment(dash_id: str, body: CommentCreate, request: Request = None, user = Depends(get_current_user)):
    # Viewer can comment? Allow editor/admin/viewer but not anon when AUTH_REQUIRED
    if user.get("id")=="anon" and os.getenv("AUTH_REQUIRED","false").lower() in ("true","1","yes"):
    ...
    audit_log("dashboard.comment", user, dashboard_id=dash_id, ip=request.client.host if request and request.client else "", extra=comment["text"][:100])
    return comment

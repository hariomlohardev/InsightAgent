...
async def slack_events(
    ...
    if signing_secret and verify_env not in ("false", "0", "no"):
        ...
        if not verify_slack_signature(
            signing_secret, x_slack_request_timestamp, body, x_slack_signature
        ):
    ...
    if not dataset_id:
        ...
        if ds:
            ...
            return {
                "status": "no dataset",
                "reply": "No dataset uploaded yet — upload a CSV in the app first.",
            }
    # Call chat service
...
async def slack_slash(request: Request):
    ...
    if not text:
        return {
            ...
            "text": "Usage: /insight <your question> — e.g., /insight top products by sales",
        }

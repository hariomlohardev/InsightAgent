...
def _register_cloud_client(email, pwd, ws_name="WS"):
    ...
    try:
        ...
        r = c.post("/api/cloud/auth/register", json={"email": email, "password": pwd, "workspace_name": ws_name})
...
def test_workspace_isolation():
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".csv", delete=False, mode="w") as tmp:
            ...
            p = Path(tmp.name)
        with open(p, "rb") as f:
            r1 = c.post("/api/datasets/upload", files={"file":("a.csv", f, "text/csv")}, headers=_auth_headers(tok1))
        assert r1.status_code == 200, r1.text
        did1 = r1.json()["id"]
        # upload to ws2
        set_workspace_id(ws2)
        with open(p, "rb") as f:
            r2 = c.post("/api/datasets/upload", files={"file":("b.csv", f, "text/csv")}, headers=_auth_headers(tok2))
    ...
    try:
        ...
        for i in range(3):
            ...
            with tempfile.NamedTemporaryFile(suffix=".csv", delete=False, mode="w") as tmp:
                ...
                p = Path(tmp.name)
            with open(p, "rb") as f:
                rr = c.post("/api/datasets/upload", files={"file":(f"f{i}.csv", f, "text/csv")}, headers=_auth_headers(tok))
        ...
        with tempfile.NamedTemporaryFile(suffix=".csv", delete=False, mode="w") as tmp:
            ...
            p = Path(tmp.name)
        with open(p, "rb") as f:
            rr = c.post("/api/datasets/upload", files={"file":("f3.csv", f, "text/csv")}, headers=_auth_headers(tok))
        ...
        payload = {"type":"checkout.session.completed","data":{"object":{"client_reference_id": ws, "customer":"cus_mock","metadata":{"workspace_id": ws, "plan":"pro"}}}}
        ...
        with tempfile.NamedTemporaryFile(suffix=".csv", delete=False, mode="w") as tmp:
            ...
            p = Path(tmp.name)
        with open(p, "rb") as f:
            rr = c.post("/api/datasets/upload", files={"file":("f3_ok.csv", f, "text/csv")}, headers=_auth_headers(tok))
        assert rr.status_code == 200, rr.text
        # cleanup datasets
        ...
        with tempfile.NamedTemporaryFile(suffix=".csv", delete=False, mode="w") as tmp:
            ...
            p = Path(tmp.name)
        with open(p, "rb") as f:
            rr = c.post("/api/datasets/upload", files={"file":("q.csv", f, "text/csv")}, headers=_auth_headers(tok))
        did = rr.json()["id"]
        # chat 1 ok (50th)
        rchat = c.post("/api/chat", json={"dataset_id": did, "query":"show top 1"}, headers=_auth_headers(tok))
        ...
        if rchat.status_code == 200:
            ...
            _atomic_write_json(bp, b)
            rchat2 = c.post("/api/chat", json={"dataset_id": did, "query":"show top 1"}, headers=_auth_headers(tok))
...
def test_brand():
    ...
    try:
        ...
        # post brand
        r2 = c.post(f"/api/cloud/workspaces/{ws}/brand", json={"app_name":"MyCo","logo_url":"https://example.com/logo.png","primary_color":"#ff0000"}, headers=_auth_headers(tok))
        assert r2.status_code == 200, r2.text
        ...
        set_workspace_id(ws2)
        # remain free
        r4 = c.post(f"/api/cloud/workspaces/{ws2}/brand", json={"app_name":"Nope"}, headers=_auth_headers(tok2))
...
def test_llm_ollama_mock():
    ...
    try:
        ...
        assert r.status_code == 200
        # set ollama
        r2 = c.post("/api/cloud/llm", json={"provider":"ollama","model":"llama3.1:8b","ollama_url":"http://ollama:11434"}, headers=_auth_headers(tok))
        assert r2.status_code == 200
        assert r2.json()["provider"] == "ollama"
        # set openai BYOK
        r3 = c.post("/api/cloud/llm", json={"provider":"openai","model":"gpt-4o-mini","openai_key":"sk-test123"}, headers=_auth_headers(tok))
        assert r3.status_code == 200
...
def test_marketplace():
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".csv", delete=False, mode="w") as tmp:
            ...
            p = Path(tmp.name)
        with open(p, "rb") as f:
            rr = c.post("/api/datasets/upload", files={"file":("mkt.csv", f, "text/csv")}, headers=_auth_headers(tok))
        did = rr.json()["id"]
        r3 = c.post(f"/api/marketplace/{mid}/install", json={"dataset_id": did}, headers=_auth_headers(tok))
        assert r3.status_code == 200
...
def test_admin_stats():
    ...
    import time
    ts = int(time.time()*1000)%100000
    c.post("/api/auth/register", json={"email":f"viewer_{ts}@ex.com","password":"pass12345","role":"viewer"})
    ...
    try:
        ...
        for i in range(4):
            ...
            with tempfile.NamedTemporaryFile(suffix=".csv", delete=False, mode="w") as tmp:
                ...
                p = Path(tmp.name)
            with open(p, "rb") as f:
                rr = c.post("/api/datasets/upload", files={"file":(f"nocloud{i}.csv", f, "text/csv")})
            assert rr.status_code == 200, rr.text

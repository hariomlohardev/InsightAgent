...
def test_audit_log():
    ...
    with open(p, "rb") as f:
        r_up = c.post(
            "/api/datasets/upload",
            files={"file": ("audit.csv", f, "text/csv")},
...
def test_cache_and_polars_optional():
    ...
    try:
        ...
        with open(p, "rb") as f:
            r = c2.post(
                "/api/datasets/upload",
                files={"file": ("polars.csv", f, "text/csv")},
                headers=_auth_headers(token_p),
            )
        did = r.json()["id"]
...
def test_queue_sync_fallback_without_redis():
    ...
    try:
        ...
        with open(p, "rb") as f:
            r = c2.post(
                "/api/datasets/upload", files={"file": ("sales.csv", f, "text/csv")}, headers=ah
            )
        assert r.status_code == 200, r.text
        did = r.json()["id"]
        ...
        r2 = c2.post(
            "/api/chat",
            json={"dataset_id": did, "query": "forecast Sales for next 3 months"},
            headers=ah,
        )

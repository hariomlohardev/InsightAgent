...
def _make_sqlite_db(tmpdb: str, table: str = "sales"):
    ...
    if table == "sales":
        ...
        con.execute("INSERT INTO sales VALUES ('North','A',100),('South','B',200),('North','C',150)")
    elif table == "employees":
        con.execute('CREATE TABLE IF NOT EXISTS employees (Name TEXT, Department TEXT, Salary INTEGER)')
...
def test_connector_sqlite_create_and_query():
    ...
    r = client.post("/api/connectors", json={"kind":"sqlite","name":"Test SQLite","dsn":tmpdb,"table":"sales"})
    assert r.status_code == 201, r.text
    cid = r.json()["id"]
    assert r.json()["column_names"] == ["Region","Product","Sales"]
    # query
    r2 = client.post(f"/api/connectors/{cid}/query", json={"sql":"SELECT Region, SUM(Sales) as total FROM sales GROUP BY Region","limit":10})
...
def test_connector_blocked_sql():
    ...
    _make_sqlite_db(tmpdb, "sales")
    r = client.post("/api/connectors", json={"kind":"sqlite","name":"Block Test","dsn":tmpdb,"table":"sales"})
    cid = r.json()["id"]
    for sql in ["DROP TABLE sales", "DELETE FROM sales WHERE 1=1", "INSERT INTO sales VALUES ('X','Y',1)", "UPDATE sales SET Sales=0", "CREATE TABLE foo (x INT)"]:
...
def test_connector_bigquery_501():
    ...
    r = client.post("/api/connectors", json={"kind":"bigquery","name":"BQ Test","table":"project.dataset.table"})
    assert r.status_code == 201, r.text
    cid = r.json()["id"]
    # Try query - should be 501 as driver not installed or creds missing
    r2 = client.post(f"/api/connectors/{cid}/query", json={"sql":"SELECT * FROM project.dataset.table LIMIT 5"})
...
def test_connector_sheets_mock(monkeypatch=None):
    ...
    r = client.post("/api/connectors", json={"kind":"sheets","name":"Sheet Fake","sheet_url":"https://docs.google.com/spreadsheets/d/1FAKE123456789012345/edit"})
    assert r.status_code == 201, r.text
...
def test_chat_over_connector_sql_and_nl():
    ...
    _make_sqlite_db(tmpdb, "sales")
    r = client.post("/api/connectors", json={"kind":"sqlite","name":"Chat Conn","dsn":tmpdb,"table":"sales"})
    cid = r.json()["id"]
    # Raw SQL via chat
...
def test_chat_sql_blocked():
    ...
    did = r.json()["id"]
    r2 = client.post("/api/chat", json={"dataset_id": did, "query": "SELECT * FROM df; DROP TABLE df"})
...
def test_join():
    ...
    jid = rj.json()["id"]
    assert rj.json()["column_names"] == ["Region","Sales","Target"] or "Region" in rj.json()["column_names"]
...
def test_connectors_list_and_delete():
    ...
    _make_sqlite_db(tmpdb, "sales")
    r = client.post("/api/connectors", json={"kind":"sqlite","name":"List Test","dsn":tmpdb,"table":"sales"})
    cid = r.json()["id"]

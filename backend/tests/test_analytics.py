...
def test_forecast_low_data_warning():
    ...
    df = pd.DataFrame({"Date": ["2024-01-01","2024-02-01","2024-03-01"], "Sales":[100,120,110]})
    did = _upload_df(df, "small.csv")
    r = client.post("/api/chat", json={"dataset_id": did, "query": "forecast Sales for next 2 months"})
    assert r.status_code == 200
    assert r.json()["success"] == True
...
def test_what_if_decrease_by():
    ...
    r = client.post("/api/chat", json={"dataset_id": did, "query": "what if Price decreased 20% by Category"})
    assert r.status_code == 200
    assert r.json()["success"] == True
...
def test_regression_cleaning_still_works():
    ...
    r = client.post("/api/datasets/"+did+"/preview-clean", json={"query":"fill missing B with mean"})
    assert r.status_code == 200
    assert r.json()["success"] == True
    client.delete(f"/api/datasets/{did}")
...
def test_regression_sql_still_works():
    ...
    r = client.post("/api/chat", json={"dataset_id": did, "query": "SELECT * FROM df WHERE A > 1 LIMIT 2"})
    assert r.status_code == 200
    assert r.json()["success"] == True

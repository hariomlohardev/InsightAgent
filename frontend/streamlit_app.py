...
with st.sidebar:
    if st.session_state.get("token") and st.session_state.get("user"):
        ...
        with st.expander("🔐 Login (enterprise)", expanded=AUTH_REQUIRED):
            ...
            if st.button("Login", key="login_btn"):
                try:
                    r = requests.post(f"{BACKEND_URL}/api/auth/login", json={"email": email, "password": pwd}, timeout=5)
                    ...
                    st.error(str(e))
            st.caption("Default: admin@local / admin. Register via API or set AUTH_REQUIRED=false for OSS anon editor.")
            st.divider()
            ...
            if st.button("Create account", key="reg_btn"):
                try:
                    r = requests.post(f"{BACKEND_URL}/api/auth/register", json={"email": re, "password": rp, "role": rr}, timeout=5)
    ...
    if st.session_state.get("show_audit") and _is_admin():
        ...
        try:
            r = requests.get(f"{BACKEND_URL}/api/audit?limit=20", headers=_auth_headers(), timeout=5)
            if r.status_code == 200:
                for e in r.json()[:20]:
                    st.caption(f'{e.get("at","")[:19]} {e.get("user","")} {e.get("action","")} {e.get("dataset_id","")}')
            else:
...
def _try_get(path: str, timeout: int = 3):
    ...
    if "backend:8000" in BACKEND_URL:
        urls += [f"http://localhost:8000{path}", f"http://host.docker.internal:8000{path}", f"http://127.0.0.1:8000{path}"]
    for url in urls:
        try:
...
def _backend_bases():
    ...
    if "backend:8000" in BACKEND_URL:
        for alt in ["http://localhost:8000", "http://host.docker.internal:8000", "http://127.0.0.1:8000"]:
            if alt not in bases:
                bases.append(alt)
    return bases
...
def upload_dataset(file):
    ...
    for base in _backend_bases():
        try:
            ...
            r = requests.post(f"{base}/api/datasets/upload", files=files, headers=_auth_headers(), timeout=30)
            return r
        except Exception as e:
            last_exc = e
            continue
...
def chat_query(dataset_id, query, conv_id=None):
    try:
        ...
        r = requests.post(f"{BACKEND_URL}/api/chat", json=payload, headers=_auth_headers(), timeout=60)
            ...
            try:
                ...
                if job_id:
                    ...
                    with st.spinner(f'Job queued {job_id} — polling... (forecast/large)'):
                        for _ in range(20):
                            time.sleep(1)
                            pr = requests.get(f"{BACKEND_URL}/api/jobs/{job_id}", headers=_auth_headers(), timeout=5)
...
def create_dashboard(dataset_id, name, desc=""):
    try:
        r = requests.post(f"{BACKEND_URL}/api/dashboards", json={"dataset_id": dataset_id, "name": name, "description": desc}, headers=_auth_headers(), timeout=10)
        return r
    except:
        return None
...
def add_widget_to_dash(dash_id, payload):
    try:
        r = requests.post(f"{BACKEND_URL}/api/dashboards/{dash_id}/widgets", json=payload, headers=_auth_headers(), timeout=10)
        return r
    except:
        return None
...
if _share_slug:
    ...
    if dash:
        ...
        st.markdown(f'<div class="sub-header">{dash.get("name","")} — {dash.get("description","")} · dataset <code>{dash.get("dataset_id","")}</code> · <span class="fresh-badge">public</span></div>', unsafe_allow_html=True)
                ...
                for j, col in enumerate(cols):
                    ...
                    with col:
                        with st.container(border=True):
                            st.markdown(f'<div class="widget-title">{w.get("title","Untitled")}</div>', unsafe_allow_html=True)
                            st.caption(f'Q: {w.get("query","")}')
                            if w.get("result"):
                                try:
                                    df_r = pd.DataFrame(w["result"].get("data", [])) if isinstance(w["result"].get("data"), list) and w["result"]["data"] and isinstance(w["result"]["data"][0], dict) else pd.DataFrame(w["result"].get("data", []), columns=w["result"].get("columns", []))
                                    # Fallback for data shapes
                                    if w["result"].get("columns") and not df_r.empty:
                                        pass
                                    st.dataframe(df_r.head(30), use_container_width=True, height=220)
                                except Exception:
                                    st.json(w["result"])
                            if w.get("chart"):
                                try:
                                    fig = go.Figure(w["chart"])
                                    fig.update_layout(height=280, margin=dict(l=10,r=10,t=30,b=10), font=dict(size=11))
                                    st.plotly_chart(fig, use_container_width=True)
                                except Exception:
                                    st.json(w["chart"])
                            st.caption(f"pinned {w.get('created_at','')[:16]} · v{w.get('dataset_version',0)}")
        st.stop()
    else:
...
with col_h2:
    ...
    if llm:
        ...
        css = f"llm-{prov}" if prov in ["openai","groq","gemini","claude","ollama"] else "llm-heuristic"
        st.markdown(f'<div style="text-align:right; margin-top:20px;"><span class="llm-badge {css}">{prov.upper()}: {model}</span></div>', unsafe_allow_html=True)
        if prov == "heuristic":
...
if health:
    ...
    prov_text = f"LLM: {prov} ({llm.get('model')})" if llm and llm.get("configured") else "LLM: Fallback (heuristic, no key)"
    st.success(f"✅ Backend connected — v{health.get('version','0.1.0')} | {prov_text} | Storage: {root.get('storage','') if root else ''}")
    ...
    st.warning(f"⚠️ Backend not reachable at {BACKEND_URL} — retrying... (will fallback to localhost:8000)")
...
with st.sidebar:
    ...
    if not datasets:
        ...
        if _sel_meta.get("lineage"):
            st.caption(f"🔗 Joined from {', '.join(_sel_meta.get('joined_from', _sel_meta['lineage']))} on `{_sel_meta.get('join_on')}` ({_sel_meta.get('join_how')})")
        if _sel_meta.get("type")=="connector":
            st.caption(f"🔌 Live connector • kind={_sel_meta.get('connector',{}).get('kind','')} • {_sel_meta.get('column_names',[])[:4]}")
        ...
        with col1:
            if DEMO_MODE:
                ...
                r = requests.delete(f"{BACKEND_URL}/api/datasets/{dataset_id}", headers=_auth_headers())
                if r.status_code == 200:
                    st.success("Deleted")
    ...
    if dataset_id:
        ...
        with st.expander("➕ New dashboard"):
            _nd_name = st.text_input("Name", placeholder="e.g., Sales Overview", key="new_dash_name_sidebar")
            _nd_desc = st.text_input("Description", placeholder="Optional", key="new_dash_desc_sidebar")
    ...
    if _scheds:
        for _s in _scheds[:4]:
            ...
            st.caption(f"⏰ {_s.get('name','')[:18]} • `{_s.get('cron')}` • {last} • {(_s.get('runs') or [{}])[0].get('status','')}")
    else:
...
if not details or "dataset" not in details:
    st.error(f"Failed to load dataset details for `{dataset_id}` — file may be corrupted or deleted. Tried: {', '.join(_backend_bases())} (backend:8000 is the only valid base inside Docker; localhost/host.docker.internal failures inside container are expected — browser uses host localhost via published port)")
    st.caption(f"Backend URL: {BACKEND_URL} | Inside frontend container only http://backend:8000 works — localhost/host.docker.internal are expected to fail there. Browser on host hitting http://localhost:8000 with longer timeout working proves backend is up but profile is slow (>5s). Now retrying with 30s timeout. Also try: `docker exec $(docker ps -qf name=frontend) curl -m 15 http://backend:8000/api/datasets/{dataset_id}`")
    ...
    if st.button("📋 Show raw backend response (debug)"):
        for base in _backend_bases():
            try:
                ...
                hint = " (expected to fail inside Docker — only backend:8000 is valid there)" if base != "http://backend:8000" else ""
                st.code(f"{base}/api/datasets/{dataset_id}{hint} → {rr.status_code}\n{rr.text[:800]}")
            except Exception as e:
                hint = " (expected inside Docker)" if base in ("http://localhost:8000","http://127.0.0.1:8000","http://host.docker.internal:8000") else ""
...
try:
    ...
    st.error(f"Failed to parse dataset details for {dataset_id}: {e} | keys: {list(details.keys()) if isinstance(details, dict) else type(details)}")
    st.stop()
...
if meta.get("type")=="connector":
    st.info(f"🔌 **Live connector** • `{meta['connector'].get('kind')}` • table `{meta['connector'].get('table') or meta['connector'].get('sheet_url','')[:40]}` • Chat: type **SELECT** or ask in English (LLM → SQL). • Data is live (not uploaded).")
if meta.get("lineage"):
    st.info(f"🔗 **Joined dataset** • lineage `{meta['lineage']}` on `{meta.get('join_on')}` ({meta.get('join_how')}) • from {meta.get('joined_from')}")
...
try:
    if is_cloud and st.session_state.get("token"):
        br = requests.get(f"{BACKEND_URL}/api/cloud/workspaces/{st.session_state.get('user',{}).get('workspace_id','default')}/brand", headers=_auth_headers(), timeout=3)
        if br.status_code==200:
            b = br.json()
            if b.get("primary_color"):
                col = b["primary_color"]
                st.markdown("<style>:root{--primary:" + col + "} .stButton>button[kind=\"primary\"]{background:" + col + ";border-color:" + col + "}</style>", unsafe_allow_html=True)
            if b.get("app_name") and b["app_name"]!="InsightAgent":
                st.markdown("<div style=\"text-align:center;color:#64748b;font-size:0.8rem\">White-label: <strong>" + b.get("app_name","") + "</strong></div>", unsafe_allow_html=True)
except:
    pass
tabs_list = ["💬 Chat", "📋 Preview", "🔍 Profile", "📊 Quick", "🧹 Clean", f"📊 Dashboards ({_dash_count})", "🔌 Connect", "📈 Analytics", "⏰ Schedules"]
if is_cloud:
...
with tabs[1]:
    ...
    with c2:
        st.download_button("⬇️ Download Profile JSON", json.dumps(profile, indent=2), file_name="profile.json", mime="application/json")
...
with tabs[0]:
    ...
    st.caption(f"Ask anything. Columns: {', '.join(profile['column_names'])} | Inferred: {profile.get('inferred_roles',{})}")
    ...
    for idx, msg in enumerate(st.session_state["messages"]):
        if msg["role"] == "user":
            ...
            with st.expander("🔍 View Generated Code & Details", expanded=False):
                ...
                c1, c2 = st.columns(2)
                with c1:
                    st.download_button(f"⬇️ Code (.py) #{idx}", msg.get("code",""), file_name=f"code_{idx}.py", mime="text/x-python", key=f"dl_code_{idx}")
                with c2:
                    prov = msg.get("intent",{}).get("provider") or msg.get("provider") or "heuristic"
            ...
            if msg.get("result"):
                ...
                try:
                    ...
                    with col_a:
                        st.download_button(f"⬇️ Result CSV #{idx}", df_res.to_csv(index=False), file_name=f"result_{idx}.csv", mime="text/csv", key=f"dl_csv_{idx}")
                    with col_b:
                        st.download_button(f"⬇️ Result JSON #{idx}", json.dumps(res, indent=2), file_name=f"result_{idx}.json", mime="application/json", key=f"dl_json_{idx}")
                except Exception as e:
                    st.json(res)
            if msg.get("chart"):
                try:
                    ...
                    # Chart download
                    st.download_button(f"⬇️ Chart JSON #{idx}", json.dumps(msg["chart"], indent=2), file_name=f"chart_{idx}.json", mime="application/json", key=f"dl_chart_{idx}")
                except Exception as e:
            ...
            if msg.get("chart") or msg.get("result"):
                ...
                with c_pin1:
                    _pin_title = st.text_input("Pin title", value=msg.get("insight","")[:60] or f"Chart #{idx}", key=f"pin_title_{idx}", placeholder="Title for dashboard widget", label_visibility="collapsed")
                with c_pin2:
                    # Dashboard selector for pin
                    if _dashes_all:
                        _pin_dash_choice = st.selectbox("Dashboard", [d["name"] for d in _dashes_all], key=f"pin_dash_{idx}", label_visibility="collapsed")
                        _pin_dash_id = next((d["id"] for d in _dashes_all if d["name"] == _pin_dash_choice), _dashes_all[0]["id"])
                    else:
                        _pin_dash_id = None
                if st.button(f"📌 Pin to Dashboard", key=f"pin_btn_{idx}"):
                    if not _dashes_all:
                        ...
                        if r and r.status_code == 201:
                            ...
                            st.error(f"Failed to create dashboard: {r.text[:300] if r else 'no response'}")
                            st.stop()
                    payload = {
                        "query": msg.get("insight","") or st.session_state["messages"][idx-1]["content"] if idx>0 and st.session_state["messages"][idx-1]["role"]=="user" else "",
                        "code": msg.get("code",""),
                    ...
                    if r and r.status_code == 200:
                        st.toast(f"Pinned to {_pin_dash_choice if _dashes_all else 'My Dashboard'}", icon="📌")
    ...
    if st.session_state["messages"]:
        ...
        with col_clear:
            if st.button("🧹 Clear Chat"):
                if st.session_state["conversation_id"]:
                    try:
                        requests.delete(f"{BACKEND_URL}/api/chat/conversations/{st.session_state['conversation_id']}", timeout=3)
                    except:
                        pass
                st.session_state["messages"] = []
                st.session_state["conversation_id"] = None
        ...
        with col_export:
            if st.button("💾 Export Chat JSON"):
                st.download_button("Download Chat", json.dumps(st.session_state["messages"], indent=2), file_name="chat_history.json", mime="application/json", key="export_chat")

with tabs[4]:
    st.subheader("🧹 Data Cleaning & Transformation")
    st.caption("Describe cleaning in plain English — e.g., 'fill missing Price with median', 'remove duplicates', 'rename Customer_Segment to Segment', 'convert Date to datetime', 'trim whitespace in Product', 'remove outliers in Sales'")
    ...
    # Input
    clean_query = st.text_input("Cleaning instruction", value=st.session_state.get("clean_query", ""), placeholder="e.g., fill missing Sales with median", key="clean_input")
        ...
        if st.button("👁️ Preview", type="secondary", key="clean_preview_btn"):
            if not clean_query.strip():
                ...
                with st.spinner("Previewing..."):
                    try:
                        r = requests.post(f"{BACKEND_URL}/api/datasets/{dataset_id}/preview-clean", json={"query": clean_query}, timeout=30)
                        if r.status_code == 200:
                            data = r.json()
            ...
            if not preview or not preview.get("success"):
                ...
                if not clean_query.strip():
                    ...
                    with st.spinner("Applying..."):
                        try:
                            # Try apply directly
                            r = requests.post(f"{BACKEND_URL}/api/datasets/{dataset_id}/apply-clean", json={"query": clean_query, "code": preview.get("code") if preview else None}, timeout=30)
                ...
                with st.spinner("Applying..."):
                    try:
                        r = requests.post(f"{BACKEND_URL}/api/datasets/{dataset_id}/apply-clean", json={"query": clean_query, "code": preview.get("code")}, timeout=30)
                        if r.status_code == 200 and r.json().get("success"):
                            st.success(f"✅ Applied! New version {r.json().get('new_version')}")
    ...
    if preview and preview.get("success"):
        ...
        with col_diff:
            ...
            if diff:
                ...
                c1.metric("Rows", f"{diff.get('rows_before')} → {diff.get('rows_after')}", delta=diff.get('rows_after',0)-diff.get('rows_before',0))
        ...
        with col_a:
            ...
            if preview.get("preview"):
                try:
                    ...
                    st.download_button("⬇️ After CSV", df_a.to_csv(index=False), file_name="cleaned_preview.csv", mime="text/csv", key="clean_preview_csv")
                except:
...
with tabs[5]:
    ...
    st.caption("Pin charts from Chat, arrange in a grid, filter, share, export. Snapshots refresh on demand.")

    # Top actions: create / filters
    col_new1, col_new2 = st.columns([2,1])
    with col_new1:
        with st.expander("➕ New Dashboard", expanded=(len(_dashes_all)==0)):
            nd_name = st.text_input("Dashboard name", placeholder="Sales Overview", key="dash_new_name")
    ...
    with col_new2:
        ...
        if f_col != "(none)":
            try:
                ...
                uniq = df_full[f_col].dropna().unique().tolist()[:30] if f_col in df_full.columns else []
                f_vals = st.multiselect("Values", uniq, default=uniq[:5] if len(uniq)>5 else uniq, key="dash_filter_vals")
            except Exception:
                f_vals = []
    ...
    if not _dashes_all:
        st.info("No dashboards yet. Create one above, then use **📌 Pin to Dashboard** in the Chat tab.")
    else:
        # Dashboard selector
        ...
        default_idx = list(dash_names.values()).index(active_id) if active_id in dash_names.values() else 0
        sel_name = st.selectbox("Open dashboard", list(dash_names.keys()), index=default_idx, key="dash_open_select")
        ...
        if not dash:
            ...
            if is_public and slug:
                meta_cols[2].markdown(f'<span class="fresh-badge">public</span> <span class="share-box">/api/dashboards/share/{slug}</span>', unsafe_allow_html=True)
            ...
            with a3:
                if st.button("📋 Duplicate", key=f"dup_{sel_id}"):
                    r = requests.post(f"{BACKEND_URL}/api/dashboards/{sel_id}/duplicate", timeout=10)
                    if r.status_code == 200:
                        st.session_state["active_dash"] = r.json()["id"]
            ...
            if is_public and slug:
                with st.expander("Embed & share"):
                    st.code(f'<iframe src="{BACKEND_URL}/api/dashboards/share/{slug}" width="100%" height="600" style="border:1px solid #e2e8f0; border-radius:8px;"></iframe>', language="html")
                    st.caption("Frontend share (Streamlit): append `?share=<slug>` to your frontend URL")
            ...
            try:
                ...
                current_version = r_v.json().get("current_version", 0) if r_v.status_code==200 else 0
            except:
                current_version = 0

            widgets = dash.get("widgets", [])
            if not widgets:
                st.info("No widgets yet — go to **Chat** and click **📌 Pin to Dashboard** on any result.")
            else:
                ...
                for i in range(0, len(widgets), 2):
                    ...
                    for j, col in enumerate(cols):
                        ...
                        badge = '<span class="stale-badge">stale — refresh</span>' if is_stale else '<span class="fresh-badge">fresh</span>'
                        with col:
                            with st.container(border=True):
                                st.markdown(f'<div class="widget-title">{w.get("title","Untitled")} {badge}</div>', unsafe_allow_html=True)
                                st.markdown(f'<div class="widget-caption">{w.get("query","")[:120]}</div>', unsafe_allow_html=True)
                                # Table (filtered client-side)
                                df_f = _filtered_df_for_widget(w)
                                if df_f is not None:
                                    st.dataframe(df_f.head(30), use_container_width=True, height=220)
                                    st.caption(f"{len(df_f)} rows (filtered)" if f_col != "(none)" else f"{w.get('result',{}).get('rows', len(df_f))} rows")
                                elif w.get("result"):
                                    st.json(w["result"])
                                # Chart
                                if w.get("chart"):
                                    try:
                                        fig = go.Figure(w["chart"])
                                        fig.update_layout(height=260, margin=dict(l=10,r=10,t=28,b=10), font=dict(family="Inter", size=11), paper_bgcolor="white", plot_bgcolor="white")
                                        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
                                    except Exception as e:
                                        st.error(f"Chart error: {e}")
                                        st.json(w["chart"])
                                ...
                                with b1:
                                    if st.button("🔄 Refresh", key=f"refresh_{w['id']}_{sel_id}"):
                                        with st.spinner("Refreshing..."):
                                            r = requests.post(f"{BACKEND_URL}/api/dashboards/{sel_id}/widgets/{w['id']}/refresh", timeout=30)
                                            if r.status_code == 200:
                                ...
                                with b2:
                                    if st.button("🗑️ Remove", key=f"remove_{w['id']}_{sel_id}"):
                                        r = requests.delete(f"{BACKEND_URL}/api/dashboards/{sel_id}/widgets/{w['id']}", headers=_auth_headers(), timeout=5)
                                        if r.status_code == 200:
                                            st.toast("Removed", icon="🗑️")
                                            st.rerun()
                                        else:
                                            st.error(r.text[:300])
                                st.caption(f"pinned {w.get('created_at','')[:16]} · code version v{w.get('dataset_version',0)} · current v{current_version}")
                                ...
                                with st.expander(f"💬 Comments ({len(dash.get('comments',[]))})"):
                                    try:
                                        ...
                                        _cmt_text = st.text_input("Comment", placeholder="Check this metric…", key=f"cmt_{w['id']}_{sel_id}", label_visibility="collapsed")
                                        if st.button("Post", key=f"cmt_post_{w['id']}_{sel_id}"):
                                            if _cmt_text.strip():
                                                try:
                                                    rr = requests.post(f"{BACKEND_URL}/api/dashboards/{sel_id}/comments", json={"text": f"[{w.get('title','')}]: {_cmt_text.strip()}", "user": (_current_user() or {}).get("email","anon")}, headers=_auth_headers(), timeout=5)
            ...
            with st.expander(f"💬 All comments ({len(dash.get('comments',[]))})"):
                try:
                    ...
                    for cm in _all_c:
                        st.markdown(f"**{cm.get('user','anon')}** · {cm.get('created_at','')[:16]} {('↳ '+cm.get('parent_id','') if cm.get('parent_id') else '')}")
                        st.caption(cm.get("text",""))
                        st.divider()
                    _acmt = st.text_input("New comment (dashboard)", placeholder="Great dashboard!", key=f"dash_cmt_{sel_id}", label_visibility="collapsed")
                    if st.button("Post dashboard comment", key=f"dash_cmt_btn_{sel_id}"):
                        if _acmt.strip():
                            rr = requests.post(f"{BACKEND_URL}/api/dashboards/{sel_id}/comments", json={"text": _acmt.strip(), "user": (_current_user() or {}).get("email","anon")}, headers=_auth_headers(), timeout=5)
                            if rr.status_code==201:
            ...
            with st.expander("⚙️ Danger zone"):
                ...
                with cda:
                    if st.button("🗑️ Delete dashboard", key=f"del_dash_{sel_id}", type="primary"):
                        r = requests.delete(f"{BACKEND_URL}/api/dashboards/{sel_id}", headers=_auth_headers(), timeout=5)
                        if r.status_code == 200:
                            st.session_state["active_dash"] = None
...
with tabs[6]:
    ...
    st.caption("Add Postgres, MySQL, SQLite, BigQuery, or Google Sheets as a **live virtual dataset**. Then chat with SELECT or join files. OSS stores DSN plain — use env for prod (encrypted in L7).")

    c1, c2 = st.columns([1,1])
    with c1:
        with st.container(border=True):
            st.markdown("**➕ New connector**")
            kind = st.selectbox("Kind", ["postgres","mysql","sqlite","bigquery","sheets"], key="conn_kind")
            name = st.text_input("Name", placeholder="e.g., Prod Postgres or My Sheet", key="conn_name")
            dsn = ""
            table = ""
            sheet_url = ""
            creds = ""
            if kind in ("postgres","mysql"):
                dsn = st.text_input("DSN", placeholder="postgresql://user:pass@host:5432/db" if kind=="postgres" else "mysql://user:pass@host:3306/db", key="conn_dsn", type="password")
                table = st.text_input("Table (optional if you will query SQL)", placeholder="public.sales", key="conn_table")
                st.caption("⚠️ DSN stored plain in OSS. For prod, use env var — encrypted storage in L7.")
            elif kind == "sqlite":
                dsn = st.text_input("SQLite path", value=":memory:", placeholder=":memory: or /data/db.sqlite", key="conn_sqlite_path")
                table = st.text_input("Table", placeholder="sales", key="conn_sqlite_table")
                st.caption("Demo: use `:memory:` — but for persistence use a file. CI demo loads sample CSV into sqlite.")
                if st.button("📥 Load sample_data into SQLite (demo)", key="conn_demo_sqlite"):
                    try:
                        ...
                        _con.close()
                        st.success(f"Loaded samples into {_db}. Now create connectors with dsn={_db} table=sales or employees")
                    except Exception as e:
                        st.error(str(e))
            elif kind == "sheets":
                sheet_url = st.text_input("Google Sheets share link", placeholder="https://docs.google.com/spreadsheets/d/<ID>/edit", key="conn_sheet_url")
                st.caption("Sheet must be **Anyone with link — Viewer**. Private sheets need OAuth (L7). Fetched via export CSV, cached 60s.")
            elif kind == "bigquery":
                table = st.text_input("Table", placeholder="project.dataset.table", key="conn_bq_table")
                creds = st.text_area("Service JSON (or set GOOGLE_APPLICATION_CREDENTIALS env)", placeholder='{"type":"service_account",...}', key="conn_bq_creds", height=80)
                if not creds:
                    st.caption("Requires `pandas-gbq` + `google-cloud-bigquery` — not installed in OSS demo, returns 501 with message.")
            if st.button("Create connector", type="primary", key="conn_create"):
                if not name.strip():
                    st.toast("Name required", icon="⚠️")
                else:
                    payload = {"kind": kind, "name": name.strip()}
                    if dsn: payload["dsn"] = dsn
                    if table: payload["table"] = table
                    if sheet_url: payload["sheet_url"] = sheet_url
                    if creds: payload["credentials_json"] = creds
                    try:
                        r = requests.post(f"{BACKEND_URL}/api/connectors", json=payload, timeout=15)
                        if r.status_code == 201:
                            j = r.json()
                            st.success(f"Connected: {j.get('id')} — {j.get('column_names')} · {j.get('rows')} rows")
    ...
    with c2:
        with st.container(border=True):
            ...
            st.caption("All queries are read-only — INSERT/UPDATE/DROP blocked. Uses DuckDB validation.")
            ...
            if not _conn_list:
                ...
                q_conn_names = {f"{c['name']} ({c['kind']}) • {c['id'][:6]}": c["id"] for c in _conn_list}
                q_sel = st.selectbox("Connector", list(q_conn_names.keys()), key="conn_q_sel")
                q_cid = q_conn_names[q_sel]
                sql = st.text_area("SQL (SELECT / WITH only)", value="SELECT * FROM df LIMIT 5", height=100, key="conn_sql")
                ...
                with col_test:
                    if st.button("Test connection", key="conn_test_btn"):
                        try:
                            r = requests.post(f"{BACKEND_URL}/api/connectors/{q_cid}/test", timeout=10)
                            if r.status_code==200:
                ...
                with col_run:
                    if st.button("Run query", type="primary", key="conn_run"):
                        try:
                            r = requests.post(f"{BACKEND_URL}/api/connectors/{q_cid}/query", json={"sql": sql, "limit": q_limit}, timeout=15)
                            if r.status_code==200:
                                j = r.json()
                                ...
                                try:
                                    ...
                                    if len(_df.columns)>=2:
                                        try:
                                            fig = px.bar(_df.head(20), x=_df.columns[0], y=_df.columns[1], title="Query preview")
                                            st.plotly_chart(fig, use_container_width=True)
                                        except: pass
                                except: st.json(j["preview"])
    ...
    with st.container(border=True):
        ...
        st.caption("Select 2–3 file datasets (or connectors) → join on a common column via DuckDB. Creates a new dataset with lineage badge. Then chat or dashboard from the joined data.")
        ...
        if not datasets:
            ...
            _join_opts = {f"{d['original_filename']} ({d['rows']} rows) • {d['id'][:6]}": d["id"] for d in datasets}
            sel_ids = st.multiselect("Pick 2–3 datasets", list(_join_opts.keys()), max_selections=3, key="join_sel")
            # Find common columns
            common = set()
            if len(sel_ids) >= 2:
                ...
                if common:
                    ...
                    st.warning(f"No common columns between {sel_ids}. Pick datasets that share a key (e.g., Region, Date).")
                    on = st.text_input("Join on (manual)", key="join_on_manual")
                how = st.selectbox("How", ["left","inner","right","outer"], index=0, key="join_how")
                if st.button("Join → create new dataset", type="primary", key="join_btn"):
                    if len(ids)<2:
                        ...
                        try:
                            r = requests.post(f"{BACKEND_URL}/api/datasets/join", json={"ids": ids, "on": on.strip(), "how": how}, timeout=20)
                            if r.status_code==200:
                                j=r.json()
                                st.success(f"Joined → {j['id']}: {j['rows']} rows, lineage {j.get('lineage')} — select it above to chat")
    ...
    st.divider()
    st.caption("💡 **NL→SQL**: When a live connector is selected above, Chat treats English as SQL. With a key (Groq/OpenAI) it translates via LLM; without a key it falls back to heuristic groupby + tip: *Add GROQ_API_KEY for full NL→SQL or type SELECT.*")

with tabs[7]:
    st.subheader("Analytics — Why, Outliers, Segments & Forecast")
    st.caption("One-click deep dives. All use read-only `df` via safe sandbox; forecast works even on 24 rows (naive fallback if `statsforecast` missing).")
    ...
    with a4:
        ...
        if st.button("🔮 Forecast", use_container_width=True, key="an_fc_btn"):
            ...
            if f_freq == "W":
                st.session_state["pending_query"] = f"forecast {f_metric} for next {f_periods} weeks"
            elif f_freq == "D":
    ...
    st.caption("Tip: Run in **Chat** tab — results show table + chart. Pin any analytics result to Dashboards. Forecast shows band + MAE/RMSE when ≥20 history points.")

with tabs[8]:
    st.subheader("Schedules & Automation")
    st.caption("Schedule a dashboard as PDF to email/Slack on cron. Threshold alerts fire when metric drops >10%. Reports bundle widgets + markdown. Comments live on dashboards. Slack bot answers via `/insight`.")
    ...
    with col_s1:
        with st.container(border=True):
            ...
            # Pick dashboard
            try:
                _all_d = requests.get(f"{BACKEND_URL}/api/datasets", timeout=3).json() if datasets else []
            except:
                _all_d = []
            ...
            sched_dash = st.selectbox("Dashboard (or leave blank for query)", ["(query)"] + list(dash_opts.keys()), key="sched_dash")
            if sched_dash == "(query)":
            ...
            # Cron helper
            cron = st.text_input("Cron", value="0 9 * * 1", placeholder="0 9 * * 1 (Mon 9am)", key="sched_cron")
            # Presets
            ...
            if c_presets[3].button("Every 5m", key="cron_5m"):
                ...
                st.rerun()
            st.caption("Cron: `m h dom mon dow` — 0 9 * * 1 = Mon 9am. Use `0 9 * * *` for daily 9am.")
            channel = st.selectbox("Channel", ["email","slack","both"], key="sched_channel")
            to_addr = st.text_input("To", placeholder="you@example.com or https://hooks.slack.com/..." if channel=="email" else "Slack webhook URL", key="sched_to")
            # Threshold optional
            with st.expander("Threshold alert (optional)"):
                ...
                thr_metric = st.text_input("Metric keyword (for future)", placeholder="Sales", key="thr_metric")
            if st.button("Create schedule", type="primary", key="sched_create"):
                if not sched_name.strip():
                    ...
                    payload = {"name": sched_name.strip(), "cron": cron.strip(), "channel": channel, "to": to_addr.strip()}
                    if sched_dash != "(query)":
                    ...
                    if thr_enabled:
                        payload["threshold"] = {"pct": thr_pct, "direction": thr_dir, "metric": thr_metric}
                    try:
    ...
    with col_s2:
        with st.container(border=True):
            ...
            if not scheds:
                ...
                for s in scheds:
                    with st.container(border=True):
                        st.markdown(f"**{s.get('name','')}** • `{s.get('cron')}` • {s.get('channel')} → `{s.get('to','')[:28]}`")
                        st.caption(f"ID {s['id']} • dashboard {s.get('dashboard_id') or s.get('query','')[:24]} • enabled {s.get('enabled')}")
                        ...
                        with b1:
                            if st.button("Run now", key=f"sched_run_{s['id']}"):
                                with st.spinner("Running..."):
                                    try:
                                        rr = requests.post(f"{BACKEND_URL}/api/schedules/{s['id']}/run", timeout=30)
                                        if rr.status_code==200:
                                            st.success(f"Ran: {rr.json().get('status')} — {rr.json().get('detail','')[:120]}")
                                            st.toast("Ran", icon="✅")
                                            st.rerun()
                                        else:
                        ...
                        with b3:
                            if st.button("Delete", key=f"sched_del_{s['id']}"):
                                try:
                                    rr = requests.delete(f"{BACKEND_URL}/api/schedules/{s['id']}", headers=_auth_headers(), timeout=5)
                                    if rr.status_code==200:
                                        st.toast("Deleted", icon="🗑️")
                                        st.rerun()
                                    else:
    ...
    with st.container(border=True):
        ...
        # Pick dashboard
        rep_dash_sel = st.selectbox("Dashboard", list(dash_opts.keys()) if dash_opts else ["(no dashboards)"] , key="rep_dash_sel")
        ...
        if rep_dash_id:
            try:
                _rep_dash = requests.get(f"{BACKEND_URL}/api/dashboards/{rep_dash_id}", timeout=5).json()
            ...
            if _widgets:
                ...
                md = st.text_area("Markdown block (optional)", placeholder="# Executive summary\n- Insight 1\n- Forecast shows +12% next quarter", height=100, key="rep_md")
                if st.button("Create report", type="primary", key="rep_create"):
                    if not rep_name.strip():
                        ...
                        if not blocks:
                            ...
                            try:
                                r = requests.post(f"{BACKEND_URL}/api/reports", json={"dashboard_id": rep_dash_id, "name": rep_name.strip(), "description": rep_desc.strip(), "blocks": blocks}, timeout=10)
                                if r.status_code==201:
        ...
        if reps:
            ...
            for rep in reps[:8]:
                ...
                with c1:
                    st.caption(f"📄 **{rep.get('name')}** • {rep['id'][:6]} • {len(rep.get('blocks',[]))} blocks • {rep.get('dashboard_id','')[:6]}")
                with c2:
                ...
                with c3:
                    if st.button("Delete", key=f"rep_del_{rep['id']}"):
                        try:
                            rr = requests.delete(f"{BACKEND_URL}/api/reports/{rep['id']}", headers=_auth_headers(), timeout=5)
                            if rr.status_code==200:
                                st.toast("Deleted", icon="🗑️")
                                st.rerun()
                        except Exception as e:
    ...
    with st.container(border=True):
        ...
        st.caption("Comments live inline in `dashboards/{id}.json` — max 100 per dashboard. Threaded via parent_id (flat for OSS).")
        if not _dashes_all:
            st.info("No dashboards — create one to comment.")
        else:
            c_dash = st.selectbox("Dashboard for comments", [d["name"] for d in _dashes_all], key="cmt_dash_sel")
            c_dash_id = next((d["id"] for d in _dashes_all if d["name"]==c_dash), _dashes_all[0]["id"])
            try:
            ...
            c_text = st.text_input("New comment", placeholder="Check Region West — looks off", key="cmt_text")
            c_parent = st.text_input("Parent ID (optional for thread)", placeholder="", key="cmt_parent")
            if st.button("Post comment", key="cmt_post"):
                if not c_text.strip():
                    st.toast("Text required", icon="⚠️")
                else:
                    try:
                        r = requests.post(f"{BACKEND_URL}/api/dashboards/{c_dash_id}/comments", json={"text": c_text.strip(), "user": c_user.strip() or "anon", "parent_id": c_parent.strip() or None}, timeout=5)
                        if r.status_code==201:
                            st.toast("Comment posted", icon="💬")
                            st.rerun()
                        else:
                        ...
                        st.error(str(e))
            st.caption("Delete via API `DELETE /api/dashboards/{id}/comments/{cid}` — UI delete shown in runs for now.")
    ...
    if len(tabs) > 9:
        with tabs[9]:
            ...
            try:
                ...
                # Billing
                br = requests.get(f"{BACKEND_URL}/api/cloud/billing", headers=_auth_headers(), timeout=5)
                if br.status_code==200:
                    b = br.json()
                    c1,c2,c3 = st.columns(3)
                    c1.metric("Plan", b.get("plan","free"))
                    c2.metric("Datasets", str(b.get("usage",{}).get("datasets",0)) + "/" + str(b.get("quotas",{}).get("datasets",3)))
                    c3.metric("Queries/mo", str(b.get("usage",{}).get("queries_this_month",0)) + "/" + str(b.get("quotas",{}).get("queries_per_month",50)))
                    # Upgrade
                    if b.get("plan")=="free":
                        if st.button("Upgrade to Pro ($19/mo)"):
                            rr = requests.post(f"{BACKEND_URL}/api/cloud/billing/checkout", json={"plan":"pro"}, headers=_auth_headers(), timeout=10)
                            if rr.status_code==200:
                ...
                with st.form("brand_form"):
                    ...
                    if st.form_submit_button("Save brand"):
                        rr = requests.post(f"{BACKEND_URL}/api/cloud/workspaces/{ws_id}/brand", json={"app_name":appn,"logo_url":logo,"primary_color":color}, headers=_auth_headers(), timeout=5)
                        if rr.status_code==200:
                ...
                try:
                    cur = requests.get(f"{BACKEND_URL}/api/cloud/llm", headers=_auth_headers(), timeout=5).json()
                except:
                    cur={}
                prov = st.selectbox("Provider", ["auto","openai","groq","gemini","claude","ollama","heuristic"], index=["auto","openai","groq","gemini","claude","ollama","heuristic"].index(cur.get("provider","auto")) if cur.get("provider") in ["auto","openai","groq","gemini","claude","ollama","heuristic"] else 0)
                model = st.text_input("Model", value=cur.get("model",""))
                okey = st.text_input("OpenAI key (BYOK)", type="password", placeholder="sk-...")
                oll_url = st.text_input("Ollama URL", value=cur.get("ollama_url","http://ollama:11434"))
                if st.button("Save LLM"):
                    rr = requests.post(f"{BACKEND_URL}/api/cloud/llm", json={"provider":prov,"model":model,"openai_key":okey,"ollama_url":oll_url}, headers=_auth_headers(), timeout=8)
                    if rr.status_code==200:
                        st.success(f"LLM set to {prov}")
                    else:
                        st.error(rr.text[:300])
                if st.button("Test Ollama"):
                    rr = requests.post(f"{BACKEND_URL}/api/cloud/llm/test", headers=_auth_headers(), timeout=5)
        ...
        with tabs[10]:
            ...
            try:
                lst = requests.get(f"{BACKEND_URL}/api/marketplace", headers=_auth_headers(), timeout=5)
                if lst.status_code==200:
                    for item in lst.json():
                        with st.container(border=True):
                            st.markdown("**" + item.get("name","") + "** — " + item.get("description",""))
                            qs = ", ".join(item.get("queries",[])[:2])
                            st.caption("queries: " + qs)
                            if st.button("Install " + item.get("id",""), key="mkt_" + item.get("id","")): 
                                rr = requests.post(BACKEND_URL + "/api/marketplace/" + item.get("id","") + "/install", json={"dataset_id": dataset_id}, headers=_auth_headers(), timeout=10)
                                if rr.status_code==200:
                                    did = rr.json().get("dashboard",{}).get("id","")[:6]
                                    st.success("Installed " + item.get("id","") + " — dashboard " + did)
                                    st.json(rr.json())
                                else:
                                    st.error(rr.text[:300])
                else:
            ...
            if versions:
                ...
                if st.button("↩️ Revert", key="revert_btn_bottom"):
                    with st.spinner(f"Reverting to v{sel_v}..."):
                        try:
                            r2 = requests.post(f"{BACKEND_URL}/api/datasets/{dataset_id}/revert", json={"version": sel_v}, timeout=10)
